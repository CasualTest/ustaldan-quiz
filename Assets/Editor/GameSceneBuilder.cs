...
public static class GameSceneBuilder
{
    ...
    [MenuItem("UstAldan Quiz/Fix Fonts — Set PTSans as TMP Default")]
    public static void SetPTSansAsDefault()
    {
        var ptSans = FindFont();
        if (ptSans == null)
        {
        ...
        // TMP Settings лежит в Resources как "TMP Settings"
    ...
    [MenuItem("UstAldan Quiz/Fix Fonts — Enable Dynamic Mode")]
    public static void FixFontsDynamic()
    {
        int count = 0;
        foreach (var g in AssetDatabase.FindAssets("t:TMP_FontAsset"))
        {
            var path = AssetDatabase.GUIDToAssetPath(g);
            if (path.StartsWith("Packages/")) continue;    // не трогаем пакеты TMP

            var font = AssetDatabase.LoadAssetAtPath<TMPro.TMP_FontAsset>(path);
            if (font == null) continue;

            // Назначить sourceFontFile, если пустой — TMP нужен .ttf для Dynamic
            if (font.sourceFontFile == null)
            {
                string dir      = Path.GetDirectoryName(path)?.Replace('\\', '/') ?? "Assets/Fonts";
                string baseName = Path.GetFileNameWithoutExtension(path)
                                      .Replace(" SDF", "").Replace("_SDF", "").Trim();
                var ttfGuids = AssetDatabase.FindAssets($"{baseName} t:Font", new[] { dir });
                if (ttfGuids.Length == 0)
                    ttfGuids = AssetDatabase.FindAssets($"{baseName} t:Font");   // поиск по всему проекту
                if (ttfGuids.Length > 0)
                {
                    var ttf = AssetDatabase.LoadAssetAtPath<Font>(
                                  AssetDatabase.GUIDToAssetPath(ttfGuids[0]));
                    if (ttf != null)
                    {
                        var soFont  = new SerializedObject(font);
                        var sfProp  = soFont.FindProperty("m_SourceFontFile");
                        if (sfProp != null) { sfProp.objectReferenceValue = ttf; soFont.ApplyModifiedProperties(); }
                    }
                }
            }

            font.atlasPopulationMode = TMPro.AtlasPopulationMode.Dynamic;
            EditorUtility.SetDirty(font);
            Debug.Log($"[FontFix] {path} → Dynamic  (source: {font.sourceFontFile?.name ?? "не найден!"})");
            count++;
        }

        AssetDatabase.SaveAssets();
        EditorUtility.DisplayDialog(
            "Шрифты — Dynamic режим",
            $"Готово! Изменено шрифтов: {count}\n\n" +
            "Теперь TMP рендерит любые символы напрямую из .ttf файла.\n" +
            "Якутские и другие символы будут отображаться корректно.",
            "OK");
    }

    // ─────────────────────────────────────────────────────────────────────
    // Диагностика: печатает состояние каждого TMP-шрифта в Console
    // ─────────────────────────────────────────────────────────────────────
    [MenuItem("UstAldan Quiz/Fix Fonts — Print Status")]
    public static void PrintFontStatus()
    {
        Debug.Log("══════════ TMP Font Status ══════════");
        foreach (var g in AssetDatabase.FindAssets("t:TMP_FontAsset"))
        {
            var path = AssetDatabase.GUIDToAssetPath(g);
            ...
            if (font == null) continue;
            bool hasB  = font.HasCharacter('б'); // б
            bool hasKh = font.HasCharacter('х'); // х
            Debug.Log($"  [{(hasB && hasKh ? "OK" : "!!")}] {font.name}\n" +
                      $"      mode   = {font.atlasPopulationMode}\n" +
                      $"      source = {(font.sourceFontFile != null ? font.sourceFontFile.name : "NULL ← проблема!")}\n" +
                      $"      'б'={hasB}  'х'={hasKh}  glyphs={font.glyphTable?.Count}  atlas={font.atlasWidth}×{font.atlasHeight}");
        }
        Debug.Log("═════════════════════════════════════");
        Debug.Log("Скопируй этот вывод и покажи, если проблема не решена.");
    }

    // ─────────────────────────────────────────────────────────────────────
    // Полная пересборка: добавляет весь Latin + Cyrillic в каждый шрифт
    // ─────────────────────────────────────────────────────────────────────
    [MenuItem("UstAldan Quiz/Fix Fonts — Full Cyrillic Rebuild")]
    public static void FixFontsFullCyrillic()
    {
        // Строка со всеми нужными символами
        var sb = new System.Text.StringBuilder();
        for (int c = 0x0020; c <= 0x007E; c++) sb.Append((char)c); // Basic Latin
        for (int c = 0x00A0; c <= 0x00FF; c++) sb.Append((char)c); // Latin-1
        for (int c = 0x0400; c <= 0x04FF; c++) sb.Append((char)c); // Cyrillic
        for (int c = 0x0500; c <= 0x052F; c++) sb.Append((char)c); // Cyrillic Supplement
        ...
        foreach (var g in AssetDatabase.FindAssets("t:TMP_FontAsset"))
        {
            ...
            // 1. Найти исходный .ttf
            Font src = font.sourceFontFile;
            if (src == null)
            {
                string dir      = Path.GetDirectoryName(path)?.Replace('\\', '/') ?? "Assets";
                string baseName = Path.GetFileNameWithoutExtension(path)
                                      .Replace(" SDF", "").Replace("_SDF", "").Trim();
                var tg = AssetDatabase.FindAssets($"t:Font {baseName}", new[] { dir });
                if (tg.Length == 0) tg = AssetDatabase.FindAssets($"t:Font {baseName}");
                if (tg.Length > 0)
                    src = AssetDatabase.LoadAssetAtPath<Font>(AssetDatabase.GUIDToAssetPath(tg[0]));
            }

            if (src == null)
            {
                Debug.LogWarning($"[FontFix] {font.name}: source .ttf НЕ НАЙДЕН — пропущен!\n" +
                                 "  Открой шрифт в Inspector и назначь поле 'Source Font File' вручную.");
                skipped++;
                continue;
            }

            // 2. Назначить sourceFontFile через SerializedObject
            var soF  = new SerializedObject(font);
            var prop = soF.FindProperty("m_SourceFontFile");
            if (prop != null) { prop.objectReferenceValue = src; soF.ApplyModifiedProperties(); }

            // 3. Dynamic mode
            font.atlasPopulationMode = AtlasPopulationMode.Dynamic;

            // 4. Добавить символы (БЕЗ ClearFontAssetData — иначе теряются уже рабочие глифы)
            try
            {
                font.TryAddCharacters(allChars, out string missing);
                string info = string.IsNullOrEmpty(missing)
                    ? "все символы добавлены"
                    : $"не добавлено {missing.Length} симв. (нет в .ttf?)";
                Debug.Log($"[FontFix] ✓ {font.name}  src={src.name}  {info}");
            }
            catch (System.Exception ex)
            {
                Debug.LogWarning($"[FontFix] {font.name}: TryAddCharacters не работает: {ex.Message}\n" +
                                 "  Попробуй обновить TextMeshPro через Package Manager.");
            }
        ...
        AssetDatabase.SaveAssets();
        EditorUtility.DisplayDialog(
            "Full Cyrillic Rebuild",
            $"Готово: {done}  Пропущено (нет .ttf): {skipped}\n\n" +
            "Смотри Console (Window → General → Console) для деталей.",
            "OK");
    }

    // ─────────────────────────────────────────────────────────────────────
    // Заменяет шрифт во всех сценах с LiberationSans → PTSans SDF
    // ─────────────────────────────────────────────────────────────────────
    [MenuItem("UstAldan Quiz/Fix Fonts — Update All Scenes")]
    public static void FixFontsInScenes()
    {
        var ptSans = FindFont();
        if (ptSans == null)
        {
            EditorUtility.DisplayDialog("Ошибка", "PTSans SDF не найден в Assets/Fonts/!", "OK");
            return;
        }
        Debug.Log($"[FontFix] Целевой шрифт: {ptSans.name}");
        ...
        {
            if (!File.Exists(scenePath)) { Debug.LogWarning($"[FontFix] Не найдена: {scenePath}"); continue; }
            ...
            {
                if (tmp.font == ptSans) continue;
                Debug.Log($"[FontFix]   {scene.name}/{tmp.gameObject.name}: {tmp.font?.name ?? "null"} → {ptSans.name}");
                tmp.font = ptSans;
                EditorUtility.SetDirty(tmp);
                fixedInScene++;
            }

            if (fixedInScene > 0) EditorSceneManager.SaveScene(scene);
            Debug.Log($"[FontFix] {scene.name}: исправлено {fixedInScene} компонентов");
            totalFixed += fixedInScene;
        }

        EditorUtility.DisplayDialog(
            "Fix Fonts in Scenes",
            $"Исправлено TMP_Text: {totalFixed}\nСмотри Console для деталей.",
            "OK");
    }
    ...
    {
        // Берём PTSans-шрифт с наибольшим числом глифов (наиболее полный)
        ...
        // Fallback только если PTSans вообще нет в проекте
