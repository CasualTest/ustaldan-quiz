...
namespace UstAldanQuiz.UI
{
    public class SettingsPageUI : MonoBehaviour
    {
        ...
        private static readonly Color ColorLangTxt = new Color(0.10f, 0.16f, 0.10f);
        ...
        private void Start()
        {
            ...
            btnLangRu?.onClick.AddListener(() => LocaleManager.CurrentLanguage = LocaleManager.LangRu);
            btnLangSah?.onClick.AddListener(() => LocaleManager.CurrentLanguage = LocaleManager.LangSah);
            RefreshLangButtons();
        ...
        {
            SetLangBtn(btnLangRu,  LocaleManager.CurrentLanguage == LocaleManager.LangRu);
            SetLangBtn(btnLangSah, LocaleManager.CurrentLanguage == LocaleManager.LangSah);
        }

        private void SetLangBtn(Button btn, bool active)
        {
            if (btn == null) return;
            btn.image.color = active ? ColorLangOn : ColorLangOff;
            var tmp = btn.GetComponentInChildren<TMP_Text>();
            if (tmp != null) tmp.color = active ? Color.white : ColorLangTxt;
        }
        ...
        {
            if (toggle.targetGraphic is Image img) img.color = isOn ? ColorOn : ColorOff;
